...
async def get_active_cards():
    ...
    for card in cards:
        ...
        member = next((m for m in ["โดม", "ไอซ์", "พี", "3ก"] if m in list_name), None)
        if not member:
            continue
...
class CustomTimeModal(discord.ui.Modal, title="⏰ กรอกเวลาเอง"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        raw = self.เวลา.value.strip()
        # รองรับทั้ง 21:25 และ 21.25
        raw = raw.replace(".", ":")
        try:
            parts = raw.split(":")
            ...
            await interaction.response.send_message("❌ รูปแบบเวลาไม่ถูกต้อง เช่น 13:30 หรือ 21.25", ephemeral=True)
            return
...
class MemberSelect(discord.ui.Select):
    def __init__(self):
        options = [
            ...
            discord.SelectOption(label="🔵 3ก",   value="3ก",   description="งานกลาง / มอบหมายร่วม"),
        ]
...
class MemberForStatusSelect(discord.ui.Select):
    def __init__(self):
        options = [
            ...
            discord.SelectOption(label="🔵 3ก",   value="3ก"),
        ]
...
async def fetch_member_cards(member: str):
    ...
    r = await loop.run_in_executor(None, partial(
        requests.get,
        f"{TRELLO_BASE}/boards/{TRELLO_BOARD_ID}/cards",
        **{"params": {"key": TRELLO_API_KEY, "token": TRELLO_TOKEN}}
    ))
    cards = []
    for c in r.json():
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.edit_original_response(
                content=f"✅ **{card_name}** → เสร็จแล้ว! 🎉", view=None
            )
        except Exception as e:
            await interaction.edit_original_response(content=f"❌ {e}", view=None)
...
MEMBER_CHOICES = [
    ...
    app_commands.Choice(name="3ก", value="3ก"),
]
 
@tree.command(name="mywork", description="ดูงานของฉัน พร้อมกดเสร็จได้เลย")
@app_commands.describe(ชื่อ="เลือกชื่อของคุณ")
@app_commands.choices(ชื่อ=MEMBER_CHOICES)
async def mywork_command(interaction: discord.Interaction, ชื่อ: app_commands.Choice[str]):
    await interaction.response.defer(ephemeral=True)
    try:
        cards = await fetch_member_cards(ชื่อ.value)
        avatars = {"โดม": "🟣", "ไอซ์": "🟢", "พี": "🟡", "3ก": "🔵"}
        embed = discord.Embed(
            title=f"{avatars.get(ชื่อ.value,'')} งานของ {ชื่อ.value}",
            color=0x7F77DD
        )
        if not cards:
            embed.description = "ไม่มีงานค้างอยู่ 🎉"
...
# ── DM สรุปงานตอนเช้า (ส่งให้สมาชิกที่ map ไว้) ──────────────────
# ใส่ Discord user ID ของแต่ละคน เพื่อให้ bot ส่ง DM ได้
MEMBER_DISCORD_IDS = {
    "โดม": 1447848699718799390,
    "ไอซ์": 1447848730781945876,
    "พี": 1448155823623639162,
}
MORNING_DM_TIME = (10, 0)  # ส่ง DM ทุกเช้า 10:00
 
async def morning_dm():
    await client.wait_until_ready()
    while not client.is_closed():
        now = datetime.now(TZ)
        if now.hour == MORNING_DM_TIME[0] and now.minute == MORNING_DM_TIME[1]:
            for member, uid in MEMBER_DISCORD_IDS.items():
                try:
                    cards = await fetch_member_cards(member)
                    user = await client.fetch_user(uid)
                    embed = discord.Embed(
                        title=f"☀️ สวัสดีตอนเช้า {member}!",
                        description="งานของคุณวันนี้" if cards else "วันนี้ไม่มีงานค้าง 🎉",
...
async def on_ready():
    ...
    print('☀️ เปิดระบบ DM สรุปงานตอนเช้า 9:00 น. แล้ว')
