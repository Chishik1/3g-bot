...
async def get_all_cards():
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, _get_all_cards_sync)
        ...
        try:
            ...
            for card in cards:
                ...
                if 0 < diff_hours <= 24:
                    ...
                    embed.add_field(name="ลิงก์", value=f"[เปิดใน Trello แล้วทำซะ]({card['url']})", inline=False)
...
class MemberForStatusSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            content=f"**งานของไอ้ {member}** — เลือกงานที่จะเปลี่ยนสถานะ:",
            view=view
        )
...
@app_commands.describe(ชื่อ="เลือกชื่อของมึง")
...
async def mywork_command(interaction: discord.Interaction, ชื่อ: app_commands.Choice[str]):
    ...
    try:
        ...
        embed = discord.Embed(
            title=f"{AVATARS.get(ชื่อ.value,'')} งานของไอ้ {ชื่อ.value} ที่ยังค้างอยู่",
            color=0x7F77DD
        )
        if not cards:
            embed.description = "ไม่มีงานค้างวะ ดีงาม 🎉"
